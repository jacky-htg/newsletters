...
if (Core_Array::getRequest('action')) {
	...
	if (intval(Core_Array::getRequest('export_type')) == 1) {
		...
		if (is_array($arr)) {
			$contents = '';	
			foreach ($arr as $row) {
				$contents .= "" . $row['email'] . " " . $row['name'] . " " . $row['address'] . " " . $row['city'] . " " . $row['province'] . " " . $row['country'] . " " . $row['zipcode'] . " " . $row['phone'] . " " . $row['company'] . " " . $row['content'] . "\r\n";
			}
		}
		...
		$aSheet->setCellValue('J1',core::getLanguage('str', 'content'));
		...
		foreach ($arr as $row) {
			...
			$aSheet->setCellValue('I'.$i, $row['company']);
                        $aSheet->setCellValue('J'.$i, $row['content']);
		}
...
$tpl->assign('TITLE', 'Export Feedbacks');
