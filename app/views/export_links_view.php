...
if (Core_Array::getRequest('action')) {
	$arr = $data->getLinksList(Core_Array::getRequest('id_cat'));
	
	if (intval(Core_Array::getRequest('export_type')) == 1) {
		$ext = 'txt';
		$filename = 'linksexport.txt';			
			
		if (is_array($arr)) {
			$contents = '';	
			foreach ($arr as $row) {
				$contents .= "" . $row['email'] . " " . $row['name'] . " " . $row['address'] . " " . $row['city'] . " " . $row['province'] . " " . $row['country'] . " " . $row['zipcode'] . " " . $row['phone'] . " " . $row['company'] . " " . $row['url'] . " " . $row['ip'] . " " . $row['country_links'] . " " . $row['city_links'] . " " . $row['created_at'] . "\r\n";
			}
		}
	} elseif (intval(Core_Array::getRequest('export_type')) == 2) {
		$ext = 'xls';
		$filename = 'linksexport.xls';
		...
		$aSheet->setCellValue('J1',core::getLanguage('str', 'url'));
                $aSheet->setCellValue('K1',core::getLanguage('str', 'ip'));
                $aSheet->setCellValue('L1',core::getLanguage('str', 'country'));
                $aSheet->setCellValue('M1',core::getLanguage('str', 'city'));
                $aSheet->setCellValue('N1',core::getLanguage('str', 'created_at'));
		...
		foreach ($arr as $row) {
			...
			$aSheet->setCellValue('J'.$i, $row['url']);
                        $aSheet->setCellValue('K'.$i, $row['ip']);
                        $aSheet->setCellValue('L'.$i, $row['country_links']);
                        $aSheet->setCellValue('M'.$i, $row['city_links']);
                        $aSheet->setCellValue('N'.$i, $row['created_at']);
		}
		...
		$aSheet->getColumnDimension('K')->setWidth(15);
                ...
                $aSheet->getColumnDimension('M')->setWidth(15);
                $aSheet->getColumnDimension('N')->setWidth(15);
	...
	if (intval(Core_Array::getRequest('zip')) == 2){
		...
		header('Content-Disposition: attachment; filename=linksexport.zip');
...
$tpl->assign('TITLE_PAGE', core::getLanguage('title_page', 'export_links'));
$tpl->assign('TITLE', core::getLanguage('title', 'export_links'));
